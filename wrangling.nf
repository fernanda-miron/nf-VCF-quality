...
The VCF filtration pipeline

- A tool for filtrating samples and variants with high missing data
...
def helpMessage() {
	log.info"""
  ==========================================
  The VCF filtrating pipeline
  v${version}
  ==========================================

	Usage:

	nextflow run ${pipeline_name}.nf --vcf_path <path to input 1> --snp_cut <path to input 2> --id_cut <path to input 3> [--output_dir path to results ]
...
*/
pipeline_name = "nf_VCF_quality"

/*
  Initiate default values for parameters
  to avoid "WARN: Access to undefined parameter" messages
*/
params.vcf_path = false // default is false to not trigger help message
params.snp_cut = false
params.id_cut = false
...
/* Check if the input directory is provided
    ...
    and this test fails
*/
if ( !params.vcf_path | !params.snp_cut | !params.id_cut ) {
  log.error " Please provide the --vcf_path AND --snp_cut AND --id_cut \n\n" +
...
*/
params.output_dir = file(params.vcf_path).getParent() //!! maybe creates bug, should check
...
def pipelinesummary = [:]
/* log parameter values beign used into summary */
pipelinesummary['Input data']			= params.vcf_path
pipelinesummary['Input data']			= params.snp_cut
pipelinesummary['Input data']			= params.id_cut
...
vcf = Channel.fromPath("${params.vcf_path}/*.vcf")
snp_cut = Channel.from("${params.snp_cut}")
id_cut = Channel.from("${params.id_cut}")
script = Channel.fromPath("quality.R")
other_script = Channel.fromPath("final_quality.R")

/* Import modules
*/
 include {
   tmp_files; filtering; vcf_filter;
	 tmp_files as tmp_filter; final_check} from './nf_modules/modules.nf'
 ...
 workflow  {
	 p1 = tmp_files(vcf)
	 p2 = filtering(script, p1, snp_cut, id_cut)
	 p3 = vcf_filter(vcf, p2.all_id_tsv, p2.all_snp_tsv)
	 p4 = tmp_filter(p3)
	 p5 = final_check(other_script, p4, snp_cut, id_cut)
 }
